  ...
  const renderSelect = (
    ...
    <>
      <Select value={value} onValueChange={onChange}>
        <SelectTrigger className={`rounded-xl border px-3 py-2 text-sm shadow-sm focus:ring-1 focus:ring-ring focus:outline-none ${value && getColorSelect(value)}`}>
        ...
        <SelectContent>
          {options.map((opt) => (
            <SelectItem key={opt.id} value={opt.id} className="cursor-pointer px-3 py-2 text-sm rounded-md focus:bg-muted transition-colors hover:bg-black/30">
              {opt.name}
            </SelectItem>
          ))}
        </SelectContent>
    ...
    <Dialog open={open} onOpenChange={(isOpen) => setOpen(isOpen)}>
      <DialogContent>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-4">
          ...
          <Input type="text" {...register('description')} placeholder="Description" className="rounded-xl border px-3 py-2 text-sm shadow-sm focus:ring-2 focus:ring-ring focus:outline-none" />
