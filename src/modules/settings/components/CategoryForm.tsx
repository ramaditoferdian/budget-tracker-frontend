...
export default function CategoryForm() {
  ...
  const onSubmit = (data: CategoryFormValues) => {
    ...
    const payload = {
      ...data,
      name: `${selectedEmoji ? selectedEmoji : ''} ${data.name}`.trim(), // concatenate emoji to name
    };
    ...
    createMutation.mutate(payload, {
      ...
      onError: (err: any) => {
        const message = err?.response?.data?.errors?.message || 'Gagal menambahkan kategori';
        toast.error(message);
      },
    });
  };
    ...
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
      <div className="grid gap-3">
        ...
        <div className="flex flex-row gap-x-2">
          <EmojiPickerDialog />
          <Input {...register('name')} placeholder="Nama kategori (cth: Makan)" />
        </div>
